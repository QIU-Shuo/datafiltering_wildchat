#!/usr/bin/env python3
"""
WildChat Data Visualization Tool

Creates an interactive browser-based visualization of the processed WildChat dataset.
...
- Double-circle markers for high-scoring samples (avg >= 3)
...
def compute_avg_score(record: Dict[str, Any]) -> Optional[float]:
    """Compute average score for a sampled record."""
    if not record.get('is_sampled', False):
        return None
...
def create_visualization(
    ...
    cluster_ids = set(r['cluster_id'] for r in data)
    max_cluster = max(c for c in cluster_ids if c >= 0)
    cluster_colors = get_cluster_colors(max_cluster + 1)

    # Categorize records
    filtered_indices = []
    passed_not_sampled_indices = []
    sampled_low_score_indices = []
    sampled_high_score_indices = []

    for i, record in enumerate(data):
        if not record.get('filter_passed', True):
            filtered_indices.append(i)
        elif not record.get('is_sampled', False):
            passed_not_sampled_indices.append(i)
        else:
            avg_score = compute_avg_score(record)
            if avg_score is not None and avg_score >= 3:
                sampled_high_score_indices.append(i)
            else:
                sampled_low_score_indices.append(i)

    print(f"  Filtered (grey): {len(filtered_indices)}")
    print(f"  Passed, not sampled: {len(passed_not_sampled_indices)}")
    print(f"  Sampled, low score: {len(sampled_low_score_indices)}")
    print(f"  Sampled, high score: {len(sampled_high_score_indices)}")
    ...
    if passed_not_sampled_indices:
        for cluster_id in sorted(cluster_ids):
            indices = [i for i in passed_not_sampled_indices if data[i]['cluster_id'] == cluster_id]
            if not indices:
                continue
    ...
    # 3. Add sampled low score points (colored by cluster)
    if sampled_low_score_indices:
        for cluster_id in sorted(cluster_ids):
            indices = [i for i in sampled_low_score_indices if data[i]['cluster_id'] == cluster_id]
            if not indices:
                continue
            ...
            has_legend = any(
                i in passed_not_sampled_indices and data[i]['cluster_id'] == cluster_id
    ...
    # 4. Add sampled high score points (double circle)
    if sampled_high_score_indices:
        for cluster_id in sorted(cluster_ids):
            indices = [i for i in sampled_high_score_indices if data[i]['cluster_id'] == cluster_id]
            if not indices:
                continue
            ...
            has_legend = any(
                (i in passed_not_sampled_indices or i in sampled_low_score_indices)
                and data[i]['cluster_id'] == cluster_id
                for i in passed_not_sampled_indices + sampled_low_score_indices
            )
            ...
            fig.add_trace(go.Scatter(
                ...
                name=f'Cluster {cluster_id} (high score)' if cluster_id >= 0 else 'Noise (high score)',
                customdata=custom_data,
                hoverinfo='none',
                legendgroup=f'cluster_{cluster_id}',
                showlegend=not has_legend
...
<body>
    ...
    <div id="sidePanel">
        ...
        <div class="legend-hint">
            Grey = Filtered | Colored = Clusters | Black border = High score (avg &ge; 3)
        </div>
    </div>
    ...
    <script>
        ...
        function displayRecord(record) {{
            ...
            // Scores section (if sampled)
            if (record.is_sampled) {{
                html += '<div class="info-section">';
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualize processed WildChat data in browser',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python visualize_data.py -i test_output.parquet
  python visualize_data.py -i test_output.parquet -o custom_viz.html --perplexity 50
  python visualize_data.py -i test_output.parquet --no-open
        """
    )

    parser.add_argument('-i', '--input', required=True,
                        help='Input parquet file from process_wildchat.py')
    ...
    print("=" * 60)
    print("WILDCHAT DATA VISUALIZATION")
    print("=" * 60)
